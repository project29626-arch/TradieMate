// Structured Output Schema definitions for Gemini API
...
export interface TaskItem {
  ...
  dueDate?: string;
  assignee?: string;
  completed?: boolean; // Tracking status in client app
}
...
// REST API of Gemini requires schema formats slightly different from @google/genai SDK:
// - type should be lowercase/uppercase (usually uppercase works, but we should make it standard)
// - properties are defined inside properties object
export const QuoteLineItemSchema = {
  type: 'OBJECT',
  properties: {
    description: { type: 'STRING', description: 'Description of the material or labor item. Australian spelling (e.g. Labour instead of Labor).' },
